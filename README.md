...
### تغيير الإعدادات (config.json)

يمكنك الآن تعديل الإعدادات بسهولة من خلال ملف `config.json` دون الحاجة لتعديل الكود:

```json
{
    "sheet_name": "FINAL_FULL_DATA",
    "columns": {
        "content": 0,  # Column A
        "image": 1,    # Column B
        "status": 2    # Column C
    },
    "batch_size": 3    # عدد البوستات في كل تشغيل
}
```
...
| Column A (Content) | Column B (Image Link) | Column C (Status) |
|-------------------|-----------------------|-------------------|
| نص البوست الأول... | http://.../image1.jpg | Done |
| نص البوست الثاني... | http://.../image2.jpg | |

- **العمود A**: محتوى البوست (Text)
- **العمود B**: رابط الصورة (Image Link)
- **العمود C**: حالة النشر (Status - فارغ = لم ينشر بعد، Done = تم النشر)
