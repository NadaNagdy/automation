...
في ملف `.github/workflows/main.yml`:

```yaml
schedule:
  - cron: '0 * * * *'  # كل ساعة
  # - cron: '0 */2 * * *'  # كل ساعتين
  # - cron: '0 9,12,15,18 * * *'  # في الساعة 9 صباحاً، 12 ظهراً، 3 عصراً، 6 مساءً
  # - cron: '0 9 * * *'  # مرة يومياً الساعة 9 صباحاً
```
...
- ⚠️ كل تشغيل يستغرق ~1-2 دقيقة، أي حوالي 48-96 دقيقة يومياً
- ⚠️ يمكنك زيادة الحد بربط بطاقة ائتمان (مجاناً حتى 3000 دقيقة)
